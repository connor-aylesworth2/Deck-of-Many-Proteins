...
/*
 * Usage:
 *   php motifs_to_sql.php <job_id> <motif_dir>
 */

if ($argc < 3) {
    fwrite(STDERR, "Usage: php motifs_to_sql.php <job_id> <motif_dir>\n");
    exit(1);
}
...
if ($jobId <= 0) {
    fwrite(STDERR, "Error: job_id must be a positive integer.\n");
    exit(1);
}

if (!is_dir($motifDir)) {
    fwrite(STDERR, "Error: directory not found: $motifDir\n");
    exit(1);
}

$files = glob($motifDir . '/*.tsv');
if (!$files) {
    fwrite(STDERR, "No TSV files found in $motifDir\n");
    exit(1);
}

$findSequenceStmt = $pdo->prepare("
    SELECT sequence_id
    FROM sequences
    WHERE job_id = :job_id
...
$checkDuplicateStmt = $pdo->prepare("
    ...
    WHERE job_id = :job_id
      ...
      AND motif_name = :motif_name
      AND start_pos = :start_pos
      AND end_pos = :end_pos
    LIMIT 1
");

$insertMotifStmt = $pdo->prepare("
    INSERT INTO motif_hits
    (job_id, sequence_id, motif_name, prosite_accession, start_pos, end_pos, hit_score, hit_description)
    VALUES
    (:job_id, :sequence_id, :motif_name, :prosite_accession, :start_pos, :end_pos, :hit_score, :hit_description)
");
...
try {
    ...
    foreach ($files as $filePath) {
        ...
        if ($handle === false) {
            echo "  Skipped file: could not open\n";
            $skipped++;
            continue;
        }
        ...
        while (($row = fgetcsv($handle, 0, "\t")) !== false) {
            ...
            if ($seqName === '' || $motif === '' || $start <= 0 || $end <= 0) {
                $skipped++;
                continue;
            }

            $findSequenceStmt->execute([
                ':job_id' => $jobId,
                ':accession' => $seqName
            ]);
            ...
            if (!$sequence) {
                echo "  No matching sequence for accession $seqName\n";
                $skipped++;
                continue;
            }
