...
if ($argc < 3) {
    fwrite(STDERR, "Usage: php populate_sequences.php <job_id> <input_fasta>\n");
    exit(1);
}
...
if (!file_exists($input_fasta)) {
    fwrite(STDERR, "Error: FASTA file not found: $input_fasta\n");
    exit(1);
}
...
if ($handle === false) {
    fwrite(STDERR, "Error: could not open FASTA file.\n");
    exit(1);
}
...
while (($line = fgets($handle)) !== false) {
    ...
    if ($line[0] === '>') {
        if ($current_header !== null) {
            $records[] = [$current_header, $current_seq];
        }
        $current_header = $line;
        $current_seq = '';
    } else {
