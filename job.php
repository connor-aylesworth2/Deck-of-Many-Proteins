...
if ($job_id <= 0) {
    echo '<main class="container"><p class="error">Invalid job ID.</p></main>';
    include 'footer.php';
    exit;
}
...
if (!$job) {
    echo '<main class="container"><p class="error">Job not found.</p></main>';
    include 'footer.php';
    exit;
}
...
$motif_stmt = $pdo->prepare("
    SELECT
        ...
        s.organism_name,
        mh.motif_name,
        mh.prosite_accession,
        mh.start_pos,
        mh.end_pos,
        mh.hit_score
    FROM motif_hits mh
    JOIN sequences s ON mh.sequence_id = s.sequence_id
    WHERE mh.job_id = ?
...
foreach ($files as $file) {
    if ($file['file_type'] === 'plotcon_png') {
        $plot_path = $file['file_path'];
        break;
    }
}
?>
    ...
    <section class="card">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <?php
                    echo $row['hit_score'] !== null
                        ? htmlspecialchars((string)$row['hit_score'])
                        : 'N/A';
                    ?>
                </td>
            </tr>
